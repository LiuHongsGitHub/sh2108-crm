...
<body>
	...
	<div style="position: relative; top: 10px; left: 40px;">
		...
		<div id="remarkDiv" style="background-color: #E6E6E6; width: 870px; height: 90px;">
			<form role="form" style="position: relative;top: 10px; left: 10px;" id="remarkForm">
...
<script>
	...
	function saveCustomerRemark() {
		$.get("/crm/workbench/customer/saveRemark",{'noteContent':$("#remark").val(),'customerId':'${id}'},function (data) {
			//返回值data是resultVo
			if (data.ok) {
				alert(data.message);
				document.querySelector("#remarkForm").reset();
				refresh();
			}
		},'json');
	}
	function refresh() {
		$.get(
				"/crm/workbench/customer/queryDetail",
				{'id':'${id}'},
				function (data) {
					//customer
					var customer = data;
					...
					$('.remarkDiv').remove();
					//取出客户的集合备注
					var customerRemarkList = customer.customerRemarkList;
					//拼接
					for(var i in customerRemarkList){
						var remark = customerRemarkList[i];
						$("#remarkDiv").before("<div class=\"remarkDiv\" style=\"height: 60px;\">\n" +
								...
								"\t\t\t\t<font color=\"gray\">联系人</font> <font color=\"gray\">-</font> <b>"+customer.name+"</b> <small style=\"color: gray;\"> "+remark.createTime+" 由"+customer.createBy+"</small>\n" +
								"\t\t\t\t<div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">\n" +
	...
	$("#updateRemarkBtn").click(
			function () {
				$.post(
						"/crm/workbench/customer/updateRemark",
						{"id": $('#edit-id').val(),
	...
	function deleteRemark(id){
		$.get("/crm/workbench/customer/deleteRemark",{'remarkId':id},function (data) {
			//返回值data是resultVo
			if (data.ok) {
				alert(data.message);
				//删除备注的div元素
