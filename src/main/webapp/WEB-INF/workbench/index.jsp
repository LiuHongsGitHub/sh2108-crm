        ...
        <ul>
            <li class="dropdown user-dropdown">
                    ...
                    <c:when test="${not empty user.img}">
                        <a href="javascript:void(0)" style="text-decoration: none; color: white;"
                           class="dropdown-toggle" data-toggle="dropdown">
                            <img src="${user.img}" style="width: 40px;height: 40px;border-radius:80% ">
                ...
                <ul class="dropdown-menu">
                    <li><a href="/crm/settings/index.html"><span class="glyphicon glyphicon-wrench"></span> 系统设置</a>
                    </li>
...
<div class="modal fade" id="editPwdModal" role="dialog">
    <div class="modal-dialog" role="document" style="width: 70%;">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="editUser()">更新</button>
            </div>
        </div>
    </div>
</div>
...
<div id="center" style="position: absolute;top: 50px; bottom: 30px; left: 0px; right: 0px;">
    ...
    <div id="navigation" style="left: 0px; width: 18%; position: relative; height: 100%; overflow:auto;">
        <ul id="no1" class="nav nav-pills nav-stacked">
            <li class="liClass"><a href="/crm/toView/workbench/main/index.jsp" target="workareaFrame"><span
...
<script>
    ...
    $('#photo').change(function () {
        $.ajaxFileUpload({
                url: '/crm/settings/user/fileUpload?time=' + new Date(), //用于文件上传的服务器端请求地址
                fileElementId: 'photo', //文件上传域的ID
                dataType: 'json', //返回值类型 一般设置为json
                success: function (data, status) {
                    if (data.ok) {
                        alert(data.message);
                        //可以把图片的存储路径设置到隐藏域中<input type="hidden" />
                        $('#img').val(data.t);
                    } else {
                        alert(data.message);
                    }
    ...
    /*校验原密码是否正确，当光标移出输入框内就校验*/
    $("#oldPwd").blur(function () {
        /*当移出光标，请求取出输入的密码*/
        $.post(
            "/crm/settings/user/verifyOldPwd",
            {"oldPwd": $(this).val()},
            function (data) {
                if (!data.ok) {
                    alert(data.message);
    ...
    /*对新密码和旧密码进行比较*/
    var newPwd = document.getElementById("newPwd");/!*js方式获取代码*!/
    ...
    /*点击更新，将密码重置，将新密码取出放入user对象，所以ajax请求只需要传值就行*/
    function editUser() {
