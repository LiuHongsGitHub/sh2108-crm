					...
					<form class="form-horizontal" role="form" id="save-form">
                        ...
                        <div style="position: relative;top: 20px;">
                            <div class="form-group">
                                ...
                                <div class="col-sm-10" style="width: 81%;">
                                    <textarea class="form-control" rows="1" id="create-address" name="address">北京大兴区大族企业湾</textarea>
                                </div>
                            </div>
                        </div>
					</form>
				...
				<div class="modal-body">
					<form class="form-horizontal" role="form" id="edit-form">
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="width: 300px;">
								<input type="text" class="form-control" id="edit-mphone" value="12345678901" name="phone">
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="width: 300px;">
								<input type="text" class="form-control" id="edit-customerName" name="customerId" placeholder="支持自动补全，输入客户不存在则新建" value="动力节点">
							</div>
						</div>
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 10px;">
				<div class="btn-group" style="position: relative; top: 18%;">
				  ...
				  <button type="button" class="btn btn-default" data-toggle="modal"  onclick="editContact()"><span class="glyphicon glyphicon-pencil"></span> 修改</button>
				  <button type="button" class="btn btn-danger" onclick="deleteContact()"><span class="glyphicon glyphicon-minus"></span> 删除</button>
				</div>
			</div>
			<div style="position: relative;top: 20px;">
...
<script>
	...
	function refresh(page,pageSize) {
		$.get("/crm/workbench/contact/list",{
			'page':page,
			'pageSize':pageSize,
			'fullname':$("#query-fullname").val(),
			'owner':$("#query-owner").val(),
			'customerId':$("#query-customerName").val(),
			'source':$("#query-source").val(),
			'birth':$("#query-birth").val()
		},function (data) {
			...
			for (var i in data.list) {
				...
				$('#contact-body').append("<tr>\n" +
						"\t\t\t\t\t\t\t<td><input value="+contact.id+" onclick='childCheck()'  class='son' type=\"checkbox\"  /></td>\n" +
						"\t\t\t\t\t\t\t<td><a style=\"text-decoration: none; cursor: pointer;\" href='/crm/toView/workbench/contacts/detail?id="+contact.id+" '  >"+contact.fullname+"</a></td>\n" +
	...
	function saveOrUpdate(text) {
		...
		$.post("/crm/workbench/contact/saveOrUpdate",form,function (data) {
			//data是resultVo
			if (data.ok){
				alert(data.message);
				refresh(1,3);
	...
	function editContact() {
		...
		if (checkedLength == 1) {
			...
			alert(id)
			//请求查询
			$.get("/crm/workbench/contact/queryContact", {"id": id}, function (data) {
				//data是客户
				var contact = data;
					//data是用户集合
					//清空
					...
					$('#edit-customerId').val(contact.customerId);
		...
		if (checkedLength == 0) {
			...
			layer.alert("删除选中的"+checkedLength+"条数据吗？", {
				...
				,yes: function(index){
					...
					$.post("/crm/workbench/contact/deleteContact",{'ids':ids.join()},function (data) {
						//data:resultVo
						if(data.ok){
							alert(data.message);
							refresh(1,3);
