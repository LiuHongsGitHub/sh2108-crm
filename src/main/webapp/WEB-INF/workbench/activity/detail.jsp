        ...
        <div class="modal-dialog" role="document" style="width: 85%;">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="edit-marketActivity">修改市场活动</h4>
                </div>
                <div class="modal-body">
...
<script>
	...
	function refresh() {
		 $.get(
			"/crm/workbench/activity/queryDetail",
				{'id':'${id}'},
				function (data) {
					//返回来的是市场活动Activity
				var activity = data;
					...
					$('#describe').text(activity.describe);
					//删除class=remarkDiv元素
					$('.remarkDiv').remove();
					//取出市场活动的集合备注
					var activityRemarkList = activity.activityRemarkList;
					//拼接
					for(var i in activityRemarkList){
						...
						$("#remarkDiv").before(
								...
								"\t\t\t\t<div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">\n" +
								"\t\t\t\t\t<a class=\"myHref\"  onclick=\"editRemarkModal('"+remark.id+"')\"    href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-edit\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>\n" +
								"\t\t\t\t\t&nbsp;&nbsp;&nbsp;&nbsp;\n" +
	...
	//点击修改按钮弹出修改模态框,
	function editRemarkModal(id) {
		//先将原内容加载到模态框中，取得原内容
		var oldNote = $("#h5" + id).text();
		//放入模态框
		$("#noteContent").val(oldNote);
		$("#edit-id").val(id);
		//弹出模态框
	...
	$("#updateRemarkBtn").click(
		function () {
			$.post(
				"/crm/workbench/activity/updateRemark",
					{"id": $('#edit-id').val(),
	...
	function deleteRemark(id){
		$.get("/crm/workbench/activity/deleteRemark",{'remarkId':id},function (data) {
			//返回值data是resultVo
			if (data.ok) {
				alert(data.message);
				//删除备注的div元素
